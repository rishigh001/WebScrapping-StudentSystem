...
def f9():
    ...
    try:
        con=connect("stu_test.db")
        cursor=con.cursor()
        sql="SELECT*FROM student ORDER BY marks LIMIT5"
...
def f11():
    ...
    try:
        ...
        if unameEnter.get()!='' and umarksEnter.get()!='':
            ...
            if len(unameEnter.get())<2:
                unameEnter.delete(0,END)
                raise Exception("Length of name should not be less than2")
            name=unameEnter.get()                         
            args=(name,rno)
            ...
            if marks<0 or marks>100:
                ...
                raise Exception("Marks out of range")
            args=(marks,rno)
            sql="UPDATE student SET marks='%d'WHERE rno="""
        else:
...
def f12():
    ...
    try:
        ...
        if rno<1:
            raise Exception('Roll number cannot be or negative')
        args=(rno)
...
info,qotd='',''
try:
   socket.create_connection(("www.google.com",88))
...
lblInfo=Label(root,text=info,font=('Arial',18,'bold'),borderwidth=1,bg="#888888",relief="solid")
...
updt_st.geometry("500x500+400+180")
updt_st.withdraw()
...
del_stBack=Button(del_st,text="Back",font=('Arial',18,'bold'),command=18)
